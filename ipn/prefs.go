...
func checkIPNotSelf(st *ipnstate.Status, ip netaddr.IP) bool {
...
// ClearExitNodeInfo sets the ExitNodeID and ExitNodeIP to their zero values.
func (p *Prefs) ClearExitNodeInfo() {
	p.ExitNodeID = ""
...
func validateExitNodeIP(s string, st *ipnstate.Status) (ip netaddr.IP, err error) {
	if s == "" {
...
// specifying either an IP address or a MagicDNS host name. This method does
// not mutate ExitNodeID and will fail if ExitNodeID is already set.
func (p *Prefs) SetExitNodeIP(s string, st *ipnstate.Status) error {
	if !p.ExitNodeID.IsZero() {
		return ErrExitNodeIDAlreadySet
	}
	ip, err := validateExitNodeIP(s, st)
	if err == nil {
		p.ExitNodeIP = ip
	}
	return err
